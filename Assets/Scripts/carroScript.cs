...
public class carroScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        transform.rotation = Quaternion.Euler(-3.44f, 8.07f, -453.5f);
        maxVueltas = 3;
    ...
    void movimientoAuto()
    {
        ...
        //--- Acelerar con la tecla espacio --
        if (velocidadMax == false && velocidadMin == false && Input.GetKey(KeyCode.Space))
        {
            rb.AddForce(transform.up * velocidad);
        }
        else if(velocidadMax == true && velocidadMin == false && Input.GetKey(KeyCode.Space))
        {
            rb.AddForce(transform.up * velMax);
        }
        else if (velocidadMax == false && velocidadMin == true && Input.GetKey(KeyCode.Space))
        {
        ...
        rb.angularVelocity = (Input.GetAxis("Vertical") * torque);
    }
